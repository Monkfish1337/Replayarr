...
> **Phase 3.** Replayarr reads each Prowlarr's newest releases every 15 minutes into a release cache and matches them against wanted events and upgrades, searching the indexers only as a fallback ([docs/phase-3.md](docs/phase-3.md)). Phase 2 added quality profiles, automatic grabs and upgrades ([docs/phase-2.md](docs/phase-2.md)). Phase 1, the manual loop with Jellyfin metadata, is in [docs/phase-1.md](docs/phase-1.md).
...
- **Searching** waits until 3 hours after the event starts, then searches every enabled indexer at once. Each gets the queries SSS sent it: torrent indexers the promotion's whole torrent list (about 60 for a football fixture: scene forms, alternative club names, team codes like `MUN-SAB`, both date orders), Easynews a few distinct spellings. The search ends when every indexer has finished or the *Search Time Limit* (a minute by default) passes, whichever comes first; later answers are ignored. Indexers can be any number of Prowlarr instances, Bitmagnet (GraphQL, ordered by seeders) and Easynews. A release reported by several indexers is listed once, under the lowest *Priority* number (as in Sonarr). When nothing matches it backs off: 30 minutes, 2, 6 and 12 hours, then daily.
- **RSS** (Phase 3): every *RSS Sync Interval* minutes each Prowlarr is asked once for its newest releases, which are cached for two weeks and matched against wanted events and upgrades. Scheduled searches check the cache before searching; see [docs/phase-3.md](docs/phase-3.md).
...
- **Candidates** pass SSS's release filter and the promotion's matcher. Rejected releases stay visible in Interactive Search with the reason, such as `wrong-date` or `sports-noise`, and can only be grabbed with *Grab anyway*. A new automatic search clears the rejected ones earlier searches kept. Matches are scored from quality, source, seeders and protocol, and each score shows how it was reached.
- **Grabbing** follows the promotion's quality profile (Settings › Profiles): an automatic search sends the best allowed match at or above the profile's minimum score, and a failed download moves on to the next best. Otherwise, or from Interactive Search, choose a release yourself. Events imported below the profile's cutoff are searched again for an upgrade (every 6 hours for a week after the event) and the file is replaced; see [docs/phase-2.md](docs/phase-2.md). A torrent goes to qBittorrent and an NZB to SABnzbd. An Easynews result is a single file over HTTPS, so Replayarr's built-in downloader fetches it into that indexer's download folder (two at a time, resuming after a restart); Easynews credentials only ever go to easynews.com.
