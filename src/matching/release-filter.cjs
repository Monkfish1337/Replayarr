// Ported from SeriousSportSync (Monkfish1337/Serioussportsync @ 0706d4d), lib/sources/release-filter.js.
...
function filterSportsNoise(results, extraPatterns, options) {
  if (!Array.isArray(results)) return { results: [], dropped: [] };
  const kept = [];
  const dropped = [];
  for (const result of results) {
    const reason = rejectionReason(result.title, extraPatterns, options);
    if (reason) dropped.push({ result, reason });
