// Ported from SeriousSportSync (Monkfish1337/Serioussportsync @ 0706d4d), lib/promotion-aliases.js.
'use strict';
...
function derivePromotionAliases(name, examples) {
  ...
  add(name);
  // Some release abbreviations retain connecting words (Match Of The Day).
  // Prefer the abbreviation actually observed, even after a leading date.
  const fullAcronym = (String(name || '').match(/[A-Za-z0-9]+/g) || []).map(word=>word[0]).join('').toUpperCase();
  const observedFull = fullAcronym.length >= 2 && lines.some(line=>sceneTokens(line).includes(fullAcronym.toLowerCase()));
  add(observedFull ? fullAcronym : acronym(name));
...
function deriveSearchTitleTemplates(name, examples) {
  ...
  const learnedAliases = derivePromotionAliases(name, lines);
  for (const line of lines) {
    const cleaned = cleanReleaseTitle(line).replace(/[-]+/g, ' ').replace(/\s+/g, ' ').trim();
    const stable = stripEventStageSuffix(stablePrefix(line));
    const prefix = stable || learnedAliases.find(alias=>(' '+sceneTerm(cleaned)+' ').includes(' '+sceneTerm(alias)+' '));
    const compactDate = /\b(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\b/.exec(cleaned);
    if (compactDate) {
      const marker = prefix;
      if (marker) {
        const markerAt = cleaned.toLowerCase().indexOf(marker.toLowerCase());
        // A dated show needs only its identity and air date. Fixtures still
        // include their teams; the full show event name repeats the date.
        const eventPart = /\s(?:vs\.?|v\.?|at|@)\s/i.test(cleaned) ? ' {name}' : '';
        add((markerAt < compactDate.index ? '{promotion} {date_compact}' : '{date_compact} {promotion}') + eventPart);
      }
      continue;
    }
    if (!prefix) continue;
    ...
    if (fullDate) {
      const eventPart = stable || /\s(?:vs\.?|v\.?|at|@)\s/i.test(cleaned) ? ' {name}' : '';
      add(prefixAt >= 0 && prefixAt < fullDate.index
        ? '{promotion} {date_spaced}' + eventPart
        : '{date_spaced} {promotion}' + eventPart);
      continue;
    }
