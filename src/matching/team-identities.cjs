// Ported from SeriousSportSync (Monkfish1337/Serioussportsync @ 0706d4d), lib/team-identities.js.
'use strict';
...
// "Club" and its Iberian/Brazilian forms are pure filler: providers register
// "Club Atlético de Madrid" while releases say "Atletico Madrid". Unlike the
// initialisms beside them these carry no identity, so removing one cannot make
// a name ambiguous. Disambiguating prefixes are deliberately NOT listed —
// "AC Milan" reduced to "Milan" would start matching "Inter Milan".
const PREFIX_RE = /^(?:F\.?C\.?|F\.?K\.?|N\.?K\.?|G\.?N\.?K\.?|P\.?F\.?C\.?|A\.?F\.?C\.?|S\.?K\.?|Š\.?K\.?|C\.?F\.?|Clube|Club)\s+/i;
