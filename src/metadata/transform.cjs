// Ported from SeriousSportSync (Monkfish1337/Serioussportsync @ 0706d4d), lib/transform.js.
const promotions = require('../matching/promotions.cjs');
const config = require('./config.cjs');
const teamIdentities = require('../matching/team-identities.cjs');
...
// The shape a client should draw this item as.
//
// `ev.posterShape` is stamped at ingest, so an event stored before its
// promotion's shape changed keeps the old one for as long as it sits in the
// catalogue — the promotion definition is the live answer and the stored value
// is a snapshot. Falling back to the promotion means a shape change takes
// effect on the next catalogue read rather than the next full refresh.
//
// And 'regular' was never a Stremio poster shape at all: the spec has 'square',
// 'poster' and 'landscape'. A client given 'regular' falls back to its own
// default, which is how square team badges ended up cropped into 16:9 tiles.
function shapeFor(ev) {
  const promotion = ev && ev.id ? promotions.getByEventId(ev.id) : null;
  const shape = (promotion && promotion.posterShape) || (ev && ev.posterShape) || 'landscape';
  return ['square', 'poster', 'landscape'].includes(shape) ? shape : 'landscape';
}
...
function fromTsdb(raw, promotion) {
  ...
  const name = (typeof promotion.normaliseName === 'function')
    ...
    : rawName;
  const sourceDate = raw.dateEvent || null;
  const date = typeof promotion.correctDate === 'function'
    ? (promotion.correctDate(name, sourceDate, raw) || sourceDate)
    : sourceDate;
  const dateCorrected = date !== sourceDate;
  ...
  const ev = {
    ...
    kind,
    date,
    dateLocal: dateCorrected ? date : (raw.dateEventLocal || sourceDate),
    time: dateCorrected ? null : (raw.strTime || null),
    timestamp: dateCorrected ? null : (raw.strTimestamp || null),
...
function fromWiki(raw, promotion) {
  ...
  // Week number and season span, for leagues whose releases are keyed by week
  // rather than by date (NFL: "NFL.2025-2026.W04.Packers-Cowboys", no date
  // anywhere in the name). Only ESPN supplies these, and only for the regular
  // season; kept off the event entirely otherwise so a query builder can test
  // for presence rather than for a sentinel.
  if (Number.isInteger(raw.week) && raw.week > 0) ev.week = raw.week;
  if (raw.seasonSpan) ev.seasonSpan = String(raw.seasonSpan);
  if (raw.seasonPhase) ev.seasonPhase = String(raw.seasonPhase);
  ev.genres = promotion.genres(ev);
  return ev;
}
...
// 0.93.1 — the affix-stripped form is the one releases actually use.
//
// football-data gives three forms and only three: shortName "Man United",
// name "Manchester United FC", tla "MUN". Search-title generation drops
// FC-suffixed forms on purpose — no release group writes "Manchester United
// FC" — so the only long form in the list was being filtered out, and every
// indexer query for an EPL fixture went out as "Man United vs Ipswich Town".
//
// DARKSPORT and every other group name that release "Manchester United Vs
// Ipswich Town", so a literal text search for the short name matched nothing.
// Built-in Usenet and Easynews both returned zero for fixtures that plainly
// had releases; only Sport-Video, which matches on its own stored titles
// rather than a search string, kept working. That is what "only Sport-Video
// pulls" looked like from the outside.
//
// Stripping the affix costs one line and fixes it for every football-data
// league at once — EPL, Serie A, Ligue 1, the Championship — rather than
// needing a curated alias table per competition.
function footballDataTeamNames(team) {
  ...
  // Short name first: it is what the fixture is titled, so it stays the
  // primary identity. The long form follows, both with and without its legal
  // affix, and the tla last.
  push(team.shortName);
  push(teamIdentities.stripLegalAffixes(team.name));
  push(teamIdentities.sceneForm(team.name));
  push(team.name);
  push(team.tla);
  return out;
}
