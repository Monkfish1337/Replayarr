// Ported from SeriousSportSync (Monkfish1337/Serioussportsync @ 0706d4d), lib/sources/uefa.js.
'use strict';
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    try {
      response = await fetch(url, httpAgent.fetchOpts({
        headers: { Accept: 'application/json', 'User-Agent': 'serioussportsync/0.76.3' },
