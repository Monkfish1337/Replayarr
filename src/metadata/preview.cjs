// Ported from SeriousSportSync (Monkfish1337/Serioussportsync @ 0706d4d), lib/metadata-preview.js.
// Replayarr changes: keys come from ./config.cjs; ESPN and AEW sources added.
'use strict';
...
const defaultAdapters = {
  thesportsdb: require('./sources/thesportsdb.cjs'),
  mlb: require('./sources/mlb.cjs'),
  onefc: require('./sources/onefc.cjs'),
  footballData: require('./sources/football-data.cjs'),
  apiFootball: require('./sources/api-football.cjs'),
  uefa: require('./sources/uefa.cjs'),
  tmdb: require('./sources/tmdb.cjs'),
  jsonFeed: require('./sources/json-feed.cjs'),
  espn: require('./sources/espn.cjs'),
  aew: require('./sources/aew.cjs'),
};
...
function normalize(raw, type, promotion) {
  ...
  if (['onefc', 'mlb', 'json-feed', 'espn', 'aew'].includes(type)) return transform.fromWiki(raw, promotion);
  return null;
}
...
async function fetchRaw(definition, opts) {
  ...
  if (source.type === 'football-data') {
    const apiKey = (opts.credentials && opts.credentials.footballDataApiKey)
      || config.footballData.apiKey;
    if (!apiKey) throw new Error('football-data.org API key is not configured (Metadata › Settings)');
    if (source.teamId) {
  ...
  if (source.type === 'api-football') {
    const apiKey = (opts.credentials && opts.credentials.apiFootballApiKey)
      || config.apiFootball.apiKey;
    if (!apiKey) throw new Error('API-Football API key is not configured (Metadata › Settings)');
    const league = await adapters.apiFootball.lookupLeague({ leagueId: source.leagueId, apiKey, log });
    if (!league || !league.league) throw new Error('API-Football competition was not found');
    const seasons = adapters.apiFootball.seasonsForRange(dateFrom, dateTo);
    return adapters.apiFootball.fetchAll({
      leagueId: source.leagueId, seasons, dateFrom, dateTo, apiKey, log,
    });
  }
  if (source.type === 'uefa') {
    const seasons = adapters.uefa.seasonsForRange(dateFrom, dateTo);
    return adapters.uefa.fetchAll({
      competitionId: source.competitionId, seasons, dateFrom, dateTo, log,
    });
  }
  if (source.type === 'tmdb') {
    const apiKey = (opts.credentials && opts.credentials.tmdbApiKey)
      || (config.tmdb && config.tmdb.apiKey) || '';
    if (!apiKey) throw new Error('TMDB API key is not configured (Metadata › Settings)');
  ...
  if (source.type === 'json-feed') {
    return adapters.jsonFeed.fetchAll(source, {
      fetchImpl: opts.fetchImpl,
      timeoutMs: opts.timeoutMs,
    });
  }
  if (source.type === 'espn') {
    return adapters.espn.fetchAll({ league: source.league, dateFrom, dateTo, log });
  }
  if (source.type === 'aew') {
    return adapters.aew.fetchAll({ log });
  }
